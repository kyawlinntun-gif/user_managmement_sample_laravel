          ...
          <div class="card-body px-0 pb-2">
            <div class="row">
              <div class="col-md-6 offset-md-2">
                ...
                @enderror
                <form role="form" method="POST" action="{{ url('/admin/permissions/create') }}">
                  @csrf
                  <div class="input-group input-group-outline mb-3">
                    <input type="text" class="form-control" placeholder="Permission Name" value="{{ old('permission_name') }}" name="permission_name">
                  </div>
                  ...
                  <div>    
                    ...
                    <label>
                        <input type="radio" name="feature_id" value="{{ $feature->id }}" {{ old('feature_id') == $feature->id ? 'checked' : '' }}> {{ $feature->name }}
                    </label>
                    @endforeach
                    @error('feature_id')
                    <span class="alert alert-danger form-control" role="alert">{{ $message }}</span>
